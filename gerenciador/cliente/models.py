...
class Venda(models.Model):
    id_venda = models.BigIntegerField(primary_key=True)
...
class Fornecedor(models.Model):
    id_fornecedor = models.BigInteger(primary_key=True)
    nome = models.CharField(max_length=50)
    endereco = models.CharFiel(max_length=200)
    id_produto_fornecedor = models.ForeignKey('Produto')
    
class ComprasFornecedor(models.Model):
    id_compra_fornecedor = models.BigIntegerField(primary_key=True)
    data_compra = models.DateField()
...
class Stock(modelsModel):
    id_stock = models.BigIntegerField(primary_key=True)
    valor = models.DecimalField(max_digits=19,decimal_places=2)
